...
class SlashCommandHandlersTest {
    ...
    fun `test empty command opens modal list`() {
        ...
        val mockTypes = listOf(
            mapOf("name" to "PR Created", "typeKey" to "pr_created")
        )
        ...
        org.mockito.kotlin.doReturn(viewsOpenResponse).whenever(methodsClient).viewsOpen(any<com.slack.api.methods.request.views.ViewsOpenRequest>())
        org.mockito.kotlin.doReturn(Response.ok()).whenever(ctx).ack()

        // Act
        // Reflection is needed here to invoke the private handleCommand method since Bolt lambda registration happens at startup.
        val method = SlashCommandHandlers::class.java.getDeclaredMethod("handleCommand", SlashCommandRequest::class.java, SlashCommandContext::class.java)
    ...
    fun `test help command returns instructions`() {
        ...
        verify(ctx).ack(org.mockito.kotlin.check<String> {
            assertTrue(it.contains("/notifyme help"))
        })
    }
}
